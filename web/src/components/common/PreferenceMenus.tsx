 ...
 * Each menu lists every registered choice, marks the active one, and shows a preset's
 * palette beside its name.
 *
...
export const PreferenceMenus: React.FC = () => {
  ...
  return (
    <>
      ...
      <Tooltip title={`${t('header.language')} · ${activeLanguage ? t(activeLanguage.labelKey) : lang}`}>
        <Dropdown
          trigger={['click']}
          placement="bottomRight"
          menu={{
