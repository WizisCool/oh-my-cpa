...
const AuthFileCard: React.FC<{ file: ManagementAuthFile; selected: boolean; busy: boolean; onSelect: (checked: boolean) => void; onToggle: () => void; onDownload: () => void; onDelete: () => void }> = ({ file, selected, busy, onSelect, onToggle, onDownload, onDelete }) => {
  const t = useT();
  return (
    <article className={`terminal-panel auth-file-card ${selected ? 'is-selected' : ''}`}>
      <div className="auth-file-card-head"><input type="checkbox" checked={selected} onChange={(event) => onSelect(event.target.checked)} aria-label={t('af.select_one', { name: file.name })} /><span className={`auth-status ${file.disabled ? 'is-disabled' : file.unavailable ? 'is-unavailable' : 'is-active'}`} /><div className="auth-file-name" title={file.name}>{file.name}</div><Tag>{file.type || file.provider || 'unknown'}</Tag></div>
      <div className="auth-file-identity">{file.email || file.project_id || t('af.identity_missing')}</div>
      <div className="auth-file-meta"><span>auth: {file.auth_index || '—'}</span><span>{t('dash.success_n', { n: file.success })}</span><span>{t('dash.failure_n', { n: file.failed })}</span></div>
      {file.note && <div className="auth-file-note">{file.note}</div>}
      <div className="auth-file-footer"><span className="terminal-muted">{file.disabled ? 'DISABLED' : file.unavailable ? 'UNAVAILABLE' : 'ACTIVE'}</span><Switch size="small" checked={!file.disabled} disabled={busy || file.runtime_only} onChange={onToggle} /><Button type="text" size="small" icon={<DownloadOutlined />} disabled={busy || file.runtime_only} onClick={onDownload} aria-label={t('af.download_one', { name: file.name })} /><Popconfirm title={t('af.delete_one_title')} onConfirm={onDelete} okText={t('common.delete')} cancelText={t('common.cancel')}><Button type="text" danger size="small" icon={<DeleteOutlined />} disabled={busy || file.runtime_only} aria-label={t('af.delete_one', { name: file.name })} /></Popconfirm></div>
    </article>
  );
};
