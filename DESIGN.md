...
**The Unified Credential Workspace Density Rule.** OAuth sign-in, credential management and quota reading share one open-row collection rather than three card pages. Above 640px each record aligns identity, management metadata, quota and actions; compact mode may collapse quota to condition, plan and one primary window with an explicit full-detail action, while expanded mode keeps every returned window, dated credit, cooldown and diagnostic inline. The measured floors are eight complete compact records and three complete ordinary expanded quota records at 1440×900, with six model/group windows rendered in full and no horizontal overflow at 320px.
