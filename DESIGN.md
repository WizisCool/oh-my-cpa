...
**The One Dataset, Rendered Responsively Rule.** A list is a table on a pointer-fine viewport and labelled rows below `640px` — chosen by width, never by a control the operator has to find. The rendering changed because the measurement did: at 390px a key table measured 316 → 980px inside its card and put the reveal, copy and edit controls 664px off the screen, with no affordance saying it scrolled. Both renderings come from one column array (`web/src/components/common/phoneRowFields.ts`), so a column added to the table reaches the row and a value cannot be formatted two ways. The operator-facing table/card toggle that was removed earlier does not return: no status column, no rename modal, no second card vocabulary. See ADR 0012.
