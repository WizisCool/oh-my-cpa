...
### Status Pips & Badges
- **Status Pip**: 7×7px square, 2px radius, paired with explicit status text (e.g. `[●] 运行中`). Green = healthy, Amber = degraded/warning, Red = error, Gray = offline.
- **Cache-Rate Badge**: Pill-shaped badge featuring continuous OKLCH gradient tint fill with ≥ 4.5:1 text contrast. Displays values up to `99.9%` with one decimal place.
