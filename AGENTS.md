...
| `docs/performance-usage-audit.md` | 性能审计记录（**历史档案**） | 只追加新一次审计结论；不要改写历史数字 |
| `goal.md` | 大修复执行计划与阶段记录（**历史档案**） | 只追加"执行记录"条目；新计划另立文件，不要往里面塞 |
...
- 不要改写已 Accepted 的 ADR、历史审计数字或 `goal.md` 的阶段记录；用新文件/新条目表达变化。
