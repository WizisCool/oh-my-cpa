...
func NewHandler(cfg config.Config, repo *repository.Repository, cipher *appcrypto.Cipher, logger *slog.Logger) *Handler {
	return &Handler{
		cfg:        cfg,
		repo:       repo,
		cipher:     cipher,
...
func (p overrideRequest) validate() error {
	if p.DisplayName != nil && len([]rune(*p.DisplayName)) > 128 {
		return errors.New("display_name is too long")
	}
	if p.Notes != nil && len([]rune(*p.Notes)) > 1000 {
