...
- **OAuth 管理**：以凭据为中心统一承载登录、认证文件管理、安全字段配置、模型列表、Provider 别名以及配额读取与操作。可在控制台直接登录 Codex、Claude、Antigravity、xAI、Kimi、Devin 与 Meta Muse；若重定向回调地址在本机无法打开，可粘贴完整回调 URL 完成授权；设备码流程会显示需要确认的授权码。集合始终保留所有认证文件条目，配额只在唯一且精确的 auth index 上关联。展开密度会内联展示全部窗口、积分到期时间、冷却与诊断；紧凑密度用于批量管理，显示状态、计划和首个主窗口，并提供一键展开完整详情。
