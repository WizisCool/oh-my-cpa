...
- **Authorization Provider ID**: The id CPA uses to start an OAuth flow. It is declared per sign-in provider (for example the Claude display family starts `anthropic`), may come from a plugin's `oauth_provider`, and is resolved without fuzzy matching from the display key.
