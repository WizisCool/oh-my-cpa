...
- **Not-persisted notice**: The statement the console makes after a write the demonstration permits: the result is held in memory for the life of the instance and lost when the platform replaces it. It is shown because "the button worked" and "the change is durable" are different claims, and only the first is true here.
