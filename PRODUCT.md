...
- **Core Capabilities**:
  ...
  - Zero-CDN offline execution: Frontend assets are embedded into the Go binary for completely offline operation. The demonstration deployment holds this too: it starts no pricing sync, no capture loop, no release sweep and no plugin-logo fetch, so the only socket it opens is its own in-process fixture. A self-hosted deployment reaches exactly two outside hosts by design - `models.dev` for prices and `api.github.com` for published versions - and both are fixed at compile time, are read-only, and degrade to stale information rather than to an error; the version check can be switched off entirely for an air-gapped install;
