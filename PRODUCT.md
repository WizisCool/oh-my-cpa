...
- **成功标准**：
  ...
  2. **发现与整理闭环**：底层配置或文件变动被自动捕获至未认领（Unclaimed）队列，可平滑确认为受管资源；
...
- **核心功能**：
  - CPA 资源自动发现与未认领分拣（Unclaimed Triage Queue: Claimed, Ignored, Missing）；
  - 资源业务属性覆盖：自定义 DisplayName、Color、内置 PresetIcon / LobeIcon、Notes；
  - 凭据生命周期与真实阻断：支持 OAuth 凭据文件管理、模型权限映射；Provider 级别停用真实下发至 CPA 协议层（通过 `excluded-models: ['*']` 实现有效阻断）；
  - 监控看板与用量流：基于 CPA RESP 协议采集流式用量事件；支持 15m 实时滑动窗口、多预设周期与开闭自定义时间范围；
  - 模型计费与成本计算：models.dev 价格自动同步与本地行级定价覆盖；
...
- **视觉风格**：Terminal-flat console（极简平铺终端控制台）。
  ...
  - 等宽字体：IBM Plex Mono 为主，数据采用等宽对齐；
...
- 核心领域术语规范：`CONTEXT.md`；
- 执行契约与质量目标：`goal.md`；
- 系统架构决策记录：`docs/adr/0001-go-react-sqlite-modular-monolith.md`；
- 视觉与 Token 权威源：`docs/DESIGN.md`，代码映射见 `web/src/theme/themeConfig.ts` 与 `web/src/index.css`；
