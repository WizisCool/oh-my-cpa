...
- **Deployment Model**: Co-deployed alongside CPA on the same host, typically within a Docker Compose internal network or bound to local intranet ports. One hosted exception exists and is scoped to one purpose: a **public demonstration** served as a Cloudflare Worker, showing the same console on a generated dataset with no gateway and no credential, and refusing credential downloads, request logs, plugin execution, gateway configuration writes and anything else a public deployment must not perform (`docs/architecture.md` §13, ADR 0021, `docs/ops/cloudflare-demo.md`). It is a demonstration of the console, not of the running product: the API's responses are generated from the real handlers, so they carry the shapes a self-hosted install produces, but nothing behind them executes - no database, no capture pipeline and no authentication. It changes nothing about a self-hosted deployment. Its own code opens no outbound connection: the Worker answers from the dataset and forwards nothing, so no provider is ever contacted. The host it is deployed to does inject its own analytics beacon into the page, which is a property of that platform rather than of this code, and is recorded in ADR 0021. The binary's own demonstration mode remains, because it is what generates that dataset and how the self-hosted paths are tested without a gateway;
...
- **Core Capabilities**:
  ...
  - Unified OAuth management workspace: one credential-centred destination (`/oauth-management`) combines OAuth sign-in, auth-file lifecycle, safe-field configuration, model lists, provider aliases and quota reading/actions. Auth files own collection membership; quota joins only on a unique exact auth index, and ambiguous or missing associations remain explicit diagnostics rather than guessed matches. Expanded mode keeps every returned quota window and credit inline; compact mode is a deliberate bulk-management density with an explicit route back to the full detail. OAuth sessions survive minimizing the Connect task panel and remain independent per provider;
