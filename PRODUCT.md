...
- **Core Capabilities**:
  ...
  - Unified OAuth management workspace: one credential-centred destination (`/oauth-management`) combines OAuth sign-in, auth-file lifecycle, safe-field configuration, model lists, provider aliases and quota reading/actions. Auth files own collection membership; quota joins only on a unique exact auth index, and ambiguous or missing associations remain explicit diagnostics rather than guessed matches. A scan-first overview leads to a tabbed credential Drawer: complete quota windows and credit details, safe configuration, and models are separate tasks. OAuth sessions survive minimizing the Connect task panel and remain independent per provider;
